...
class drupalchatController extends ControllerBase {
    ...
    if (\Drupal::config('drupalchat.settings')->get('drupalchat_path_pages')) {
      ...
      $pages = mb_strtolower(\Drupal::config('drupalchat.settings')->get('drupalchat_path_pages'));
  ...
  public static function drupalchat_get_random_name() {
    $path = \Drupal::service('extension.list.module')->getPath('drupalchat') . "/guest_names/drupalchat_guest_random_names.txt";
  ...
  public function drupalchat_poll() {
    ...
    if (($message_count > 0) || ($polling_method == DRUPALCHAT_AJAX)) {
      if($user->id() > 0) {
        ...
        $messages = \Drupal::database()->query('SELECT m.message_id, m.uid1, m.uid2, m.message, m.timestamp FROM {drupalchat_msg} m WHERE (m.uid2 IN (:uid2,\'c-0\') OR m.uid1 = :uid2) AND m.timestamp > :timestamp ORDER BY m.timestamp ASC', array(':uid2' => $user->id(), ':timestamp' => $last_timestamp));
      }
      else {
        // TODO: Drupal Rector Notice: Please delete the following comment after you've made any necessary changes.
        // You will need to use `\Drupal\core\Database\Database::getConnection()` if you do not yet have access to the container here.
        $messages = \Drupal::database()->query('SELECT m.message_id, m.uid1, m.uid2, m.message, m.timestamp FROM {drupalchat_msg} m WHERE (m.uid2 IN (:uid2,\'c-0\') OR m.uid1 = :uid2) AND m.timestamp > :timestamp ORDER BY m.timestamp ASC', array(':uid2' => '0-'.drupalchatController::_drupalchat_get_sid(), ':timestamp' => $last_timestamp));
      }
  ...
  public static function _drupalchat_buddylist($uid) {
    ...
    elseif ($drupalchat_rel == DRUPALCHAT_REL_FF) {
      // TODO: Drupal Rector Notice: Please delete the following comment after you've made any necessary changes.
      // You will need to use `\Drupal\core\Database\Database::getConnection()` if you do not yet have access to the container here.
      $result = \Drupal::database()->query("SELECT * FROM {flag_friend} WHERE uid = :uid OR friend_uid = :uid", array(':uid' => $uid, ':friend_uid' => $uid));
  ...
  public static function _drupalchat_buddylist_online($buddylist) {
    ...
    if(\Drupal::config('drupalchat.settings')->get('drupalchat_enable_chatroom') == 1) {
      ...
      if(\Drupal::config('drupalchat.settings')->get('drupalchat_user_picture') == 1) {
        $users['c-0']['p'] = $base_url . '/' . \Drupal::service('extension.list.module')->getPath('drupalchat') . '/css/themes/' . \Drupal::config('drupalchat.settings')->get('drupalchat_theme') . '/images/default_room.png';
      }
    }
      ...
      if(($user->id() > 0) || (drupalchatController::_drupalchat_get_sid() != -1)) {
        ...
        if (\Drupal::config('drupalchat.settings')->get('drupalchat_polling_method') == DRUPALCHAT_COMMERCIAL) {
          ...
          if(\Drupal::currentUser()->hasPermission('access drupalchat all logs')) {
            ...
            $output .= \Drupal::service('renderer')->render($f_o)->__toString();
          }
          /*****************/
          ...
          // TODO: Drupal Rector Notice: Please delete the following comment after you've made any necessary changes.
          // You will need to use `\Drupal\core\Database\Database::getConnection()` if you do not yet have access to the container here.
          $query = \Drupal::database()->query('SELECT u.name as name, g.uid as uid, g.message as message, g.TIMESTAMP as timestamp
                       FROM (
      ...
      if(($user->id() > 0) || (drupalchatController::_drupalchat_get_sid() != -1)) {
        ...
        if (\Drupal::config('drupalchat.settings')->get('drupalchat_polling_method') == DRUPALCHAT_COMMERCIAL) {
         ...
         // $q = json_decode($result->data);
          ...
          // TODO: Drupal Rector Notice: Please delete the following comment after you've made any necessary changes.
          // You will need to use `\Drupal\core\Database\Database::getConnection()` if you do not yet have access to the container here.
          $result = \Drupal::database()->select('drupalchat_msg', 'm');
        ...
        foreach($q as $record) {
          if($oldname == $record->from_name) {
            ...
            $output .= '<div style="display:block;border-bottom: 1px solid #ccc; padding: 1% 0% 1% 0%;"></div><div style="display:block; padding-top: 1%; padding-bottom: 0%"><div style="font-size:100%; display: inline;"><a href="#">' . $record->from_name . '</a></div><div style="float:right;font-size: 70%;">' . \Drupal::service('date.formatter')->format($record->timestamp, 'long') . '</div><div style="display: block; padding-top: 1%; padding-bottom: 0%">' . Html::escape($record->message) . '</div></div>';
          }
      ...
      foreach($chat_ids as $chat_id) {
        ...
        //print_r($message);
          if((((!strpos($message->uid1,'-')) && ($message->uid1 != $user->id())) || ((strpos($message->uid1,'-')) && ($message->uid1 != '0-'.drupalchatController::_drupalchat_get_sid()))) || ($message->uid2 == 'c-0')) {
            if(!strpos($message->uid1,'-')) {
              $account = \Drupal::service('entity_type.manager')->getStorage('user')->load($message->uid1);
              $temp_msg = array('message' => Html::escape($message->message), 'timestamp' => date("H:i", $message->timestamp), 'uid1' => $message->uid1, 'name' => Html::escape(user_format_name($account)), 'uid2' => $message->uid2, 'message_id' => Html::escape($message->message_id),);
            ...
            if(!strpos($message->uid2,'-')) {
              $account = \Drupal::service('entity_type.manager')->getStorage('user')->load($message->uid2);
    ...
    if ((!empty($u->user_picture)) && (null !== $u->user_picture->first()) && (is_object($u->user_picture->first()))) {
      ...
      if(isset($source[1])) {
        ...
        if(isset($source[1])) {
          ...
          $source[0] = $base_url . '/' . \Drupal::service('extension.list.module')->getPath('drupalchat') . '/css/themes/'. $drupalchat_theme.'/images/default_avatar.png';
      ...
      if($pos !== false) {
        ...
        $drupalchat_theme = \Drupal::config('drupalchat.settings')->get('drupalchat_theme') ?: 'light';
        $source = $base_url . '/' . \Drupal::service('extension.list.module')->getPath('drupalchat') . '/css/themes/'. $drupalchat_theme.'/images/default_avatar.png';
        $url = $source;
        $pos = strpos($url, ':');
        if($pos !== false) {
          $url = substr($url, $pos+1);
  ...
  public static function drupalchat_return_profile_url() {
    ...
    if($user->isAuthenticated()) {
      $link =  Url::fromUserInput('/user/' . $user->id(), array('absolute' => TRUE))->toString();
    }
    return $link;
  }
  ...
  public static function _drupalchat_chat() {
    ...
    if ($items) {
      ...
      $no_user_item = array();
      $no_user_item[] = array(
        '#markup' => Link::fromTextAndUrl(t('No users online')->__toString(), Url::fromRoute('drupalchat.user')),
    ...
    $image = [
      '#theme' => 'image',
      '#uri' => \Drupal::service('extension.list.module')->getPath('drupalchat') . '/css/themes/' . $theme . '/images/loading.gif',
      '#width' => NULL,
      '#height' => NULL,
  ...
  public function _drupalchat_ur_autocomplete($string) {
    ...
    $last_string =  mb_strtolower(array_pop($array));
    $matches = array();
    // TODO: Drupal Rector Notice: Please delete the following comment after you've made any necessary changes.
    // You will need to use `\Drupal\core\Database\Database::getConnection()` if you do not yet have access to the container here.
    $query = \Drupal::database()->select('user_relationship_types', 'u');
