...
class ConsultaForm extends FormBase {
	...
	public function buildForm(array $form, FormStateInterface $form_state) {
		...
		$form['certificados_asistencia'] = array(
			'#type' => 'textfield',
			'#title' => $this
				->t('Documento'),
			'#size' => 60,
	...
	public function submitForm(array &$form, FormStateInterface $form_state) {
		$documento = $form_state->getValue('certificados_asistencia');
		...
		if ($documento <> null) {
			...
			if ($query) {
				...
				if ($documento <> null) {
					$response = new RedirectResponse($base_url.'/resultado-consulta-certificado/'.$documento);
					$response->send();
				}
