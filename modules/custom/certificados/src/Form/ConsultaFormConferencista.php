...
class ConsultaFormConferencista extends FormBase {
	/**
	 * {@inheritdoc}
	 */
	...
	public function buildForm(array $form, FormStateInterface $form_state) {
		...
		$form['certificados_conferencista'] = array(
			'#type' => 'textfield',
			'#title' => $this
				->t('Documento'),
			'#size' => 60,
	...
	public function submitForm(array &$form, FormStateInterface $form_state) {
		$documento = $form_state->getValue('certificados_conferencista');
		...
		if ($documento <> null) {
			...
			if ($query) {
				...
				if ($documento <> null) {
					$response = new RedirectResponse($base_url.'/resultado-consulta-certificado-conferencista/'.$documento);
					$response->send();
				}
